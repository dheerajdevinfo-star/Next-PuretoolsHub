...
function calcPPF(annualAmount: number, rate: number, tenure: number) {
  const r = rate / 100
  let balance = 0
  let totalInvested = 0
  const yearlyData: { year: number; invested: number; interest: number; balance: number }[] = []
...
function calcWithdrawal(
  yearlyData: ReturnType<typeof calcPPF>["yearlyData"],
  currentYear: number
) {
  if (currentYear < 7)
    return { eligible: false, maxAmount: 0, reason: "7th year ke baad withdrawal possible hai" }
  const refBalance = Math.min(
    yearlyData[currentYear - 5]?.balance || 0,
    yearlyData[currentYear - 4]?.balance || 0
  )
  return {
    eligible: true,
    maxAmount: Math.round(refBalance * 0.5),
    reason: "4th ya 5th year ke end balance ka 50%",
  }
}
...
export default function PPFCalculator({ ppfType }: Props) {
  ...
  const [results, setResults] = useState(
    calcPPF(config.defaultAmount, config.defaultRate, config.defaultTenure)
  )
  ...
  const investedPct =
    results.maturityAmount > 0
      ? Math.round((results.totalInvested / results.maturityAmount) * 100)
      : 0
  const circumference = 2 * Math.PI * 30
  ...
  if (ppfType === "ppf-withdrawal") {
    const withdrawal = calcWithdrawal(results.yearlyData, currentYear)
    const loanAmount = results.yearlyData[2]
      ? Math.round(results.yearlyData[2].balance * 0.25)
      : 0
        ...
        <div className="emi_card">
          ...
          <div className="slider_group">
            ...
            <input
              ...
              value={amount}
              onChange={e => { setAmount(Number(e.target.value)); handleSliderInput(e) }}
              className="emi_range"
            />
            <div className="range_hints"><span>₹500</span><span>₹1.5 lakh</span></div>
          </div>
          ...
          <div className="slider_group">
            ...
            <input
              type="range" min={6} max={9} step={0.05}
              value={rate}
          ...
          <div className="slider_group">
            ...
            <input
              ...
              value={currentYear}
              onChange={e => { setCurrentYear(Number(e.target.value)); handleSliderInput(e) }}
              className="emi_range"
            />
            <div className="range_hints"><span>Year 1</span><span>Year 15</span></div>
          </div>
                ...
                <tr key={row.year}
                  ...
                  <td style={{ color: row.year >= 7 ? "#1D9E75" : "#6B7280" }}>
                    {row.year >= 7
                      ...
                      : row.year >= 3
                        ? `Loan: ${formatINR(Math.round(row.balance * 0.25))}`
                        : "—"}
                  </td>
                </tr>
              ))}
  ...
  if (ppfType === "ppf-vs-fd") {
    ...
    const fdResults = calcFDReturns(amount, fdRate, tenure, taxSlab)
    const ppfWins =
      ppfResults.maturityAmount >
      amount * tenure + fdResults.totalInterest - fdResults.taxOnInterest
        ...
        <div className="emi_card">
          ...
          <div className="slider_group">
            ...
            <input
              ...
              value={fdRate}
              onChange={e => { setFdRate(Number(e.target.value)); handleSliderInput(e) }}
              className="emi_range"
            />
            <div className="range_hints"><span>4%</span><span>9%</span></div>
          </div>
          ...
          <div className="slider_group">
            ...
            <input
              ...
              value={tenure}
              onChange={e => { setTenure(Number(e.target.value)); handleSliderInput(e) }}
              className="emi_range"
            />
            <div className="range_hints"><span>5 years</span><span>30 years</span></div>
          </div>
        ...
        <div className="emi_card">
          ...
          <div className="slider_group">
            ...
            <input
              ...
              value={extBlocks}
              onChange={e => { setExtBlocks(Number(e.target.value)); handleSliderInput(e) }}
              className="emi_range"
            />
            <div className="range_hints"><span>1 block (5yr)</span><span>5 blocks (25yr)</span></div>
          </div>
